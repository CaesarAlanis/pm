...
async def test_call_ai_constructs_correct_request():
    ...
    with patch("app.ai.OPENROUTER_API_KEY", "test-key"):
        with patch("app.ai.httpx.AsyncClient") as mock_cls:
            mock_resp = AsyncMock()
...
async def test_call_ai_missing_key():
    from app.ai import call_ai
    with patch("app.ai.OPENROUTER_API_KEY", ""):
...
def test_ai_test_endpoint():
    with patch("app.routes.call_ai", new_callable=AsyncMock, return_value="4"):
        response = client.post("/api/ai/test", cookies=AUTH_COOKIES)
...
def test_ai_test_endpoint_no_auth():
    ...
    response = fresh.post("/api/ai/test")
    assert response.status_code == 401


def test_ai_test_endpoint_no_api_key():
    with patch("app.routes.call_ai", new_callable=AsyncMock, side_effect=ValueError("OPENROUTER_API_KEY is not set")):
        response = client.post("/api/ai/test", cookies=AUTH_COOKIES)
